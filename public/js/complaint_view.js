...
$(document).ready(function(){
  let image1=$('#image1').attr('src').split('/').pop();
  let image2=$('#image2').attr('src').split('/').pop();
  let video=$('#video1 source').attr('src').split('/').pop();
  
  let invest_date1=$('#invest_date1').text();
  let invest_detl1=$('#invest_detl1').text();
  let action_taken1=$('#action_taken1').text();
  let invst_image1=$('#invest_image1').attr('src').split('/').pop();
  let invst_image2=$('#invest_image2').attr('src').split('/').pop();
  let invst_video=$('#invest_video1 source').attr('src').split('/').pop();
  


console.log(image1,'q11')
console.log(image2,'q22')
console.log(video,'q33')
console.log(invest_date1,'invest_date1')
console.log(invest_detl1,'invest_detl1')
console.log(action_taken1,'action_taken1')
console.log(invst_image1,'invst_image1')
console.log(invst_image2,'invst_image2')
console.log(invst_video,'invst_video')
...
// $(document).ready(function() {
//   function openMediaModal(source) {
//     let mediaType = source.includes('.mp4') ? 'video' : 'img';
//     let mediaElement = mediaType === 'video' ? 
//       `<video id="modalMedia" class="img_inlarge22 w-100" controls >
//         <source src="${source}" type="video/mp4">
//         Your browser does not support the video tag.
//       </video>` :
//       `<img src="${source}" class="img-fluid" id="enlargedImage" alt="Event Photo">`;
...
$(document).ready(function(){
  ticketSts_Num_To_Txt()
});

$(document).on('change','#tkt_status_chg #refresh_div',function(){
  ticketSts_Num_To_Txt()
});
$(document).on('load','#refresh_div',function(){
  ticketSts_Num_To_Txt()
});
  ...
  $(document).on('click','#status_chg_btn',function(){
    ...
    $.ajax({
      ...
      success:function(res){
        ...
        for(let i=0; i<res.status_Data.length; i++){
          if(  res.status_Data[i].action_id === 4 || res.status_Data[i].action_id === 5){
            options += `<option value="${ res.status_Data[i].action_id }">${ res.status_Data[i].action_value }</option>`
            }
          }
        }

        else if(  emp_role==="QUILITY TEAM" ){
          if( emp_designation==="CEE" || emp_designation==="QC" ){
            for(let i=0; i<res.status_Data.length; i++){
      ...
      if( emp_role === "MANAGER" || emp_role === "EMPLOYEE" || emp_role === "QUILITY TEAM" || emp_role === "BUSINESS HEAD"){
        ...
        if( curt_tkt_status == 'Open'){
          $('#tkt_status_chg option[value="1"]').prop('disabled',true)
          $('#tkt_status_chg option[value="3"]').prop('disabled',true)
          $('#tkt_status_chg option[value="4"]').prop('disabled',true)
          $('#tkt_status_chg option[value="5"]').prop('disabled',true)
          ...
          $('#tkt_status_chg option[value="2"]').prop('disabled',true)
          $('#tkt_status_chg option[value="3"]').prop('disabled',true)
          $('#tkt_status_chg option[value="5"]').prop('disabled',false)
          $('#tkt_status_chg option[value="none"]').prop('disabled',true)
        }
...
$(document).on('change', '#tkt_status_chg', function() {
  ...
  let invest_img2 = $('#invest_img2').val();
    ...
    if (invest_date !== '' && person_meet !== '' && invst_detl !== '' && action_taken !== '' && invest_img1 !=='none' && invest_img1 !=='' && invest_img2 !=='none' && invest_img2 !=='' ) {
...
$(document).on('click', '#submit_btn', function(){
  ...
  let sales_preson_ph_no=$('#sales_preson_ph_no').text().trim();
  let ticket_created_on=$('#ticket_created_on').text().trim();
  
  
  console.log(tkt_status_chg, 'tkt_status_chg122');
  console.log(ticket_id, 'ticket_id_11');
  ...
  console.log(sales_preson_ph_no, 'sales_preson_ph_no_11');
  console.log(ticket_created_on, 'ticket_created_on_11');
  ...
  else if (tkt_status_chg == 2) {
      
    $.ajax({
      method: 'POST',
      url: '/ticket_status_inprocess',
      data: {
        tkt_status_chg:tkt_status_chg,
        ticket_id:ticket_id,
        ...
        .then(function(){
          $.ajax({            
            ...
            data:{
              ...
              sales_preson_ph_no:sales_preson_ph_no,
  ...
  let invest_date = String($('#invest_date').val().trim());
  
  let person_meet = $('#person_meet1').val().trim();
  let invst_detl = $('#invst_detl').val().trim();
  let action_taken = $('#action_taken').val().trim();
  let invest_img1 = $('#invest_img1')[0].files[0];
  let invest_img2 = $('#invest_img2')[0].files[0];
  let invest_video = $('#invest_video')[0].files[0];

  console.log(invest_date, person_meet, invst_detl, action_taken, invest_img1, invest_img2, invest_video, 'ajax data');

  
  // if( invest_date=='none' || person_meet=='' || invst_detl=='' || action_taken=='' || invest_img1=='none' || invest_img2=='none' || invest_video=='none'){
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      Swal.fire({
        title: 'Good Job!',
        text: 'Ticket Status Reopen Successfully Updated',
        icon: 'success'
      })
      .then(()=>{
        location.reload();
